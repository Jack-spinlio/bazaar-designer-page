            ...
            <TabsContent value="parts" className="space-y-4">
              <div className="mb-4">
                <div 
                  ...
                  onClick={() => setExpandWheels(!expandWheels)}
                >
                      ...
                      <div className="grid grid-cols-6 gap-2">
                        {wheelColorOptions.map((option) => (
                          <div
                            key={option.name}
                            className={`w-10 h-10 rounded-full cursor-pointer ${option.color} flex items-center justify-center ${
                              wheelColor === option.name ? 'ring-2 ring-offset-2 ring-blue-500' : ''
                            }`}
                            onClick={() => setWheelColor(option.name)}
                          >
                            {wheelColor === option.name && option.color !== 'bg-white border border-gray-300' && (
                              <div className="text-white">✓</div>
                            )}
                            {wheelColor === option.name && option.color === 'bg-white border border-gray-300' && (
                              <div className="text-black">✓</div>
              ...
              <div className="mb-4">
                <div 
                  className="flex items-center bg-gray-100 p-3 rounded-md cursor-pointer mb-2"
                  onClick={() => setExpandForks(!expandForks)}
                >
              ...
              <div className="mb-6">
                <div 
                  className="flex items-center bg-gray-100 p-3 rounded-md cursor-pointer mb-2"
                  onClick={() => setExpandWaterBottles(!expandWaterBottles)}
                >
              ...
              <div className="flex items-center justify-between p-2">
                ...
                <Switch
                  checked={showOnlyFrame}
                  onCheckedChange={setShowOnlyFrame}
                />
              </div>
            </TabsContent>
          </Tabs>
