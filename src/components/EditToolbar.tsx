                ...
                <div className="grid grid-cols-2 gap-3">
                  {shapeOptions.map(option => <div key={option.name} className={`p-3 border rounded-lg flex flex-col items-center justify-center cursor-pointer ${selectedShape === option.shape.toLowerCase() ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`} onClick={() => setSelectedShape(option.shape.toLowerCase())}>
                      <div className="w-full h-8 flex items-center justify-center mb-2">
                        ...
                        {option.shape === 'multi-ribbed' && <img src="https://dnauvvkfpmtquaysfdvm.supabase.co/storage/v1/object/public/thumbnails//Vector%20(4).svg" alt="Multi-Ribbed" className="w-16 h-8 object-contain" />}
                        {option.shape === 'soft-triangle' && <img src="https://dnauvvkfpmtquaysfdvm.supabase.co/storage/v1/object/public/thumbnails//Vector%20(4).svg" alt="Soft-Triangle" className="w-16 h-8 object-contain" />}
